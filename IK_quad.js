...
class Leg {
    ...
    updateLegDimension(robot) {
        if (robot == LSS_Robot_Model.MechDog){
            ...
            this.L1 = L1_DESKPET;    			// Abduction to rotation distance
            this.L2 = L2_DESKPET;  				// Rotation to knee distance
            this.L3 = L3_DESKPET;   		    // Knee to foot distance
            this.foot_rad = FOOT_RAD_DESKPET;	// Foot radius
        }
    }
        ...
        for (var joint = 0; joint < 3; joint++) {
            for (var joint = 0; joint < 3; joint++) {
                if (robot == LSS_Robot_Model.MechDog){
                    ...
                    this.joint_offsets[joint] = 0;
                    this.joint_minmax[0][joint] = 0;
                    this.joint_minmax[1][joint] = 0;
                }
            }
        }
    }
...
class Body{
    constructor(robot){
        ...
        this.joints = new Joints(this.model);	
        for(var i = 0; i<4; i++){
    ...
    specialMoves(){
        ...
        if (this.model == LSS_Robot_Model.MechDog){
            switch(this.sp_move){
...
export {Gait_Type, LSS_Robot_Model, Special_Moves, Rotation_Dir, Foot_Trajectory, X_MECHDOG, Z_MECHDOG, Body};