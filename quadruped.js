...
class Quadruped{
    constructor(robot_model) {
        this.speed = 4;
        this.move_flag = true;
        this.ctrlSelected = ControlMode.NoControlSelected;
        this.body = new Body(robot_model);
        this.dt = new DTime(50);
        this.changeSpeed(this.speed);
    }
    updateModel(robot_model){
        this.body.updateRobotModel(robot_model);
        for(var i = 0; i<4; i++){
            this.body.legs[i].updateLegDimension(robot_model);
        }
    }
    updateAngle(angle){
        this.move_flag = true;
